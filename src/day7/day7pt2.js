...
let instructions = lines.slice(0, lines.length - 1).map(line => new Rule(line));
let res = getResult(instructions);
instructions = lines.slice(0, lines.length - 1).map(line => new Rule(line));
instructions.filter(item => item.res === 'b')[0].arg1 = res;
